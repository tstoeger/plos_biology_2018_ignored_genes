...
def _make_and_save_prediction(j, params):
    ...
    s = StandardScaler()
    s.fit(features)
    norm_features = s.transform(features)

    clf = GradientBoostingRegressor(
        n_estimators=estimators,
        loss='huber')
    clf.fit(norm_features[first, :], target[first])

    df_out = pd.DataFrame(
        index=params['feature_names'],
        columns=['importance'],
...
def predict_via_zgbrh_importance(i, p, e, n):
    """
    Prediction via zgbrh (Z-score, Gradient-Boosting-Regression with Huber)
    ...
    if not all(df_features.index == df_target.index):
        ...
        params['i'] = i
        params['feature_names'] = df_features.columns
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Runs zgbrh for a given prediction set.')
